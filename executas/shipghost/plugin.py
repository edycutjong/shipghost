...
def call_host(method, params):
    ...
    if not _host_active and method == "sampling/createMessage":
        raise ConnectionError("Anna Host connection not active. Direct external API call fallback is disabled.")
...
def host_upload_inline(filename, mime_type, content_bytes, purpose="artifact"):
    ...
    try:
        result = call_host("host/uploadFile", {
            "mode": "inline",
            "filename": filename,
            "mime_type": mime_type,
...
def embed_texts(texts, timeout=30.0):
    ...
    if isinstance(texts, str):
        texts = [texts]
    result = call_host("embeddings/create", {"input": texts, "model": "anna-managed-v1"})
...
def image_generate(prompt, n=1, size="1024x1024"):
    if not _host_active:
        return [{"url": "https://placehold.co/1024x1024/1a1a2e/06b6d4?text=ShipGhost+PR"}]
...
def files_upload(path, content_bytes, content_type, scope="app"):
    ...
    begin = call_host("files/upload_begin", {"scope": scope, "path": path, "size_bytes": len(content_bytes), "content_type": content_type})
    if not begin:
...
def files_download_url(path, scope="app"):
    ...
    return call_host("files/download_url", {"scope": scope, "path": path}) or {"url": None}
...
def storage_list_keys(prefix="", scope="user"):
    if not _host_active:
        return {"items": [], "mock": True}
    return call_host("storage/list", {"scope": scope, "prefix": prefix}) or {"items": []}
...
def agent_session_create(label="ShipGhost Agent", ttl_seconds=600):
    ...
    return call_host("agent/session.create", {"agent_submode": "auto", "label": label, "ttl_seconds": ttl_seconds}) or {}
...
def agent_session_history(session_uuid):
    ...
    return call_host("agent/session.history", {"app_session_uuid": session_uuid}) or {"messages": []}

def agent_session_cancel(session_uuid):
    """Abort an in-flight run for an agent session."""
    if not _host_active:
        return {"ok": True, "mock": True}
    return call_host("agent/session.cancel", {"app_session_uuid": session_uuid}) or {"ok": False}
...
def host_upload_negotiate(filename, mime_type, byte_length, purpose="artifact"):
    ...
    return call_host("host/uploadFile", {
        "mode": "negotiate",
        "filename": filename,
        "mime_type": mime_type,
...
def git_semantic_group(params):
    ...
    texts = [f"{c.get('message', '')} {' '.join(c.get('filesChanged', []))}" for c in commits[:64]]
...
def pr_file_archive(params):
    ...
    if action == "save":
        ...
        result = files_upload(f"shipghost/{path}", content.encode("utf-8"), "text/markdown")
...
def git_analyze(params):
    ...
    for line in git_log_raw.strip().split("\n"):
        ...
        parts = line.split("|")
        if len(parts) >= 5:
            h, name, email, date, msg = parts[0], parts[1], parts[2], parts[3], "|".join(parts[4:])
    ...
    return {
        "branch": subprocess.check_output(["git", "branch", "--show-current"], cwd=repo_path, text=True).strip(),
...
"""
    ...
    response = call_host("sampling/createMessage", {
        "messages": [
            {
                "role": "user",
                        ...
                        "tools": [
                            {
                                ...
                                "parameters": [
                                    {"name": "repoPath", "type": "string", "required": True, "description": "Absolute path to the git repository"},
                                    {"name": "baseBranch", "type": "string", "required": False, "description": "Base branch to diff against (default: main)"}
                                ]
                            },
                            {
                                "name": "pr.ghostwrite",
                                ...
                                "parameters": [
                                    {"name": "diffSummary", "type": "object", "required": True, "description": "Diff statistics (filesChanged, insertions, deletions)"},
                                    {"name": "commits", "type": "array", "required": True, "description": "Array of commit objects from git.analyze"},
                                    {"name": "fullDiff", "type": "string", "required": True, "description": "Full git diff output"},
                                    {"name": "fileGroups", "type": "array", "required": False, "description": "Files grouped by component"}
                                ]
                            },
                            {
                                ...
                                "parameters": [
                                    {"name": "commits", "type": "array", "required": True, "description": "Array of commit objects to rewrite"}
                                ]
                            },
                            {
                                "name": "pr.sign",
                                ...
                                "parameters": [
                                    {"name": "prMarkdown", "type": "string", "required": True, "description": "PR markdown content to sign"}
                                ]
                            }
                        ]
                    },
